...
export class GameGateway {
  ...
  async handleDisconnect(client: Socket) {
    ...
    this.ListOfPlayers.delete(client.data.username);
  }
  ...
  handlePlayerPlaying(
    ...
    try {
      ...
      else if (tLobby.Instance.Info.Player1.name === client.data.username ||
        tLobby.Instance.Info.Player2.name === client.data.username)
        {
          tLobby.socketing.get(client.data.username)?.emit('isPlaying', true);
          client.emit("isPlaying", true);
          //console.log(client.connected);
          //console.log(`${client.data.username} is playing`);
        }
    } catch (e) {
